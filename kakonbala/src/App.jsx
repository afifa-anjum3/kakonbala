...
export default function App() {
  ...
  const subMap = {jewelry:"Bangles, Rings, Earrings & more",crafts:"Mandala, Canvas, Clay Art & more",clothing:"Saree, Tops, Panjabi & more"};
  const emojiMap = {jewelry:"💍",crafts:"🏺",clothing:"👗"};
  return (
    <div key={cat} onClick={()=>{setCatFilter(cat);setTab("shop");}}
      style={{ position:"relative",borderRadius:20,overflow:"hidden",cursor:"pointer",height:320 }}>
      {catProduct
        ? <img src={catProduct.imageUrl} alt={cat} style={{ width:"100%",height:"100%",objectFit:"cover",transition:"transform 0.4s",display:"block" }} />
        : <div style={{ width:"100%",height:"100%",background:"linear-gradient(135deg,rgba(173,20,87,0.2),rgba(106,27,154,0.2))",display:"flex",alignItems:"center",justifyContent:"center",fontSize:80 }}>{emojiMap[cat]}</div>
      }
      <div style={{ position:"absolute",inset:0,background:"linear-gradient(to top,rgba(20,0,30,0.8) 0%,transparent 60%)" }}/>
      <div style={{ position:"absolute",bottom:0,left:0,right:0,padding:"20px 22px" }}>
        <div style={{ fontSize:20,fontWeight:900,color:"#FFF",textTransform:"uppercase",letterSpacing:1 }}>{emojiMap[cat]} {labelMap[cat]}</div>
        <div style={{ fontSize:12,color:"rgba(255,255,255,0.75)",marginTop:3 }}>{subMap[cat]}</div>
        <div style={{ fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.85)",marginTop:8 }}>Shop Now →</div>
      </div>
    </div>
  );
})}
                  <div
                    ...
                    onMouseEnter={(e) => {
                      ...
                      e.currentTarget.style.boxShadow =
                        "0 16px 40px rgba(173,20,87,0.2)";
                    }}
                    onMouseLeave={(e) => {
          ...
          <div>
            ...
            >
              {[
                ...
                { label: t.allOrders, value: orders.length, c: PURPLE },
                { label: t.products, value: products.length, c: GOLD },
                { label: t.totalStock, value: totalStock, c: SUCCESS },
              ].map((m, i) => (
        ...
        {tab === "orders" && isAdmin && (
          <div>
            ...
            >
              {[
                ["paid", t.paid, SUCCESS, "rgba(232,245,233,0.75)"],
                ["processing", t.processing, WARN, "rgba(255,243,224,0.75)"],
                ["shipped", t.shipped, INFO, "rgba(227,242,253,0.75)"],
                [
                  "pending_payment",
                  t.pendingPay,
                  DANGER,
                ...
                <tbody>
                  ...
                  {orders.map((o) => (
                    ...
                    >
                      ...
                      <td style={{ ...TD, color: MED, fontSize: 12 }}>
                        {o.createdAt?.seconds
                          ? new Date(
                      ...
                      <td style={{ ...TD, fontSize: 12, color: MED }}>
                        {o.customer?.phone || "—"}
                      </td>
                      <td style={{ ...TD, color: MED, fontSize: 12 }}>
                        {(o.items || [])
                      ...
                      <td style={{ ...TD, fontWeight: 800, color: PRIMARY }}>
