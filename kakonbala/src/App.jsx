...
export default function App() {
  ...
  const visible = products.filter(p => {
    ...
    // Price filter
    const pPrice = p.packOptions&&p.packOptions.length>0 ? Math.min(...p.packOptions.map(o=>o.price)) : p.price;
    ...
    const pa = a.packOptions&&a.packOptions.length>0?Math.min(...a.packOptions.map(o=>o.price)):a.price;
    const pb = b.packOptions&&b.packOptions.length>0?Math.min(...b.packOptions.map(o=>o.price)):b.price;
        ...
        {tab==="home" && (
          <div>
            ...
            <div style={{ marginBottom:48 }}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:18 }}>
                {["jewelry","crafts","clothing"].map(cat => {
                  ...
                  const labelMap = {jewelry:"Jewelry / গহনা",crafts:"Crafts / ক্রাফট",clothing:"Clothing / পোশাক"};
                  const subMap = {jewelry:"Bangles, Rings & more",crafts:"Mandala, Canvas & more",clothing:"Saree, Tops & more"};
                  const emojiMap = {jewelry:"💍",crafts:"🏺",clothing:"👗"};
                  return (
                    <div key={cat} onClick={()=>{setCatFilter(cat);setTab("shop");}}
                      style={{ position:"relative",borderRadius:20,overflow:"hidden",cursor:"pointer",height:300,boxShadow:"0 8px 30px rgba(173,20,87,0.18)" }}>
                      {catProduct
                        ? <img src={catProduct.imageUrl} alt={cat} style={{ width:"100%",height:"100%",objectFit:"cover",display:"block",transition:"transform 0.4s" }} />
                        : <div style={{ width:"100%",height:"100%",background:"linear-gradient(135deg,rgba(173,20,87,0.15),rgba(106,27,154,0.15))",display:"flex",alignItems:"center",justifyContent:"center",fontSize:80 }}>{emojiMap[cat]}</div>
                      }
                      <div style={{ position:"absolute",inset:0,background:"linear-gradient(to top,rgba(20,0,30,0.82) 0%,rgba(20,0,30,0.1) 55%,transparent 100%)" }}/>
                      <div style={{ position:"absolute",bottom:0,left:0,right:0,padding:"18px 20px" }}>
                        <div style={{ fontSize:19,fontWeight:900,color:"#FFF",textTransform:"uppercase",letterSpacing:0.8 }}>{emojiMap[cat]} {labelMap[cat]}</div>
                        <div style={{ fontSize:11,color:"rgba(255,255,255,0.7)",marginTop:3 }}>{subMap[cat]}</div>
                        <div style={{ fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.9)",marginTop:8 }}>Shop Now →</div>
                      </div>
                    </div>
                  );
                })}
                ...
                {[...products].slice(0,6).map(p=>(
                  <div key={p.id} style={{ ...glass,overflow:"hidden",cursor:"pointer",transition:"transform 0.2s" }}
                    ...
                    <div style={{ height:150,position:"relative",overflow:"hidden" }}>
                      <Carousel images={p.imageUrls&&p.imageUrls.length?p.imageUrls:[p.imageUrl]} emoji={p.emoji} height={220} />
            ...
            <div id="about" style={{ marginBottom:48 }}>
              ...
              {/* Contact strip */}
              <div style={{ ...glass,background:GRAD,borderRadius:20,padding:"36px 40px",textAlign:"center" }}>
                <h3 style={{ fontSize:22,fontWeight:800,color:"#FFF",margin:"0 0 8px" }}>Get in Touch</h3>
                <p style={{ color:"rgba(255,255,255,0.85)",fontSize:14,margin:"0 0 28px" }}>Have a question or want a custom order? We'd love to hear from you!</p>
                <div style={{ display:"flex",justifyContent:"center",gap:14,flexWrap:"wrap" }}>

                  {/* WhatsApp */}
                  <a href="https://wa.me/8801920895985" target="_blank" rel="noreferrer"
                    style={{ display:"flex",alignItems:"center",gap:10,background:"#25D366",borderRadius:14,padding:"12px 20px",textDecoration:"none",cursor:"pointer",boxShadow:"0 4px 15px rgba(0,0,0,0.2)",transition:"all 0.2s" }}
                    ...
                    <div style={{ textAlign:"left" }}>
                      <div style={{ fontSize:10,color:"rgba(255,255,255,0.8)",fontWeight:600 }}>WhatsApp</div>
                      <div style={{ fontSize:12,color:"#FFF",fontWeight:700 }}>+8801920895985</div>
                    </div>
                  </a>

                  {/* Email */}
                  <a href="mailto:kakonbala.official@gmail.com"
                    style={{ display:"flex",alignItems:"center",gap:10,background:"#EA4335",borderRadius:14,padding:"12px 20px",textDecoration:"none",cursor:"pointer",boxShadow:"0 4px 15px rgba(0,0,0,0.2)",transition:"all 0.2s" }}
                    onMouseEnter={e=>e.currentTarget.style.transform="translateY(-3px)"}
                    onMouseLeave={e=>e.currentTarget.style.transform="translateY(0)"}>
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="white">
                    ...
                    <div style={{ textAlign:"left" }}>
                      <div style={{ fontSize:10,color:"rgba(255,255,255,0.8)",fontWeight:600 }}>Email</div>
                      <div style={{ fontSize:11,color:"#FFF",fontWeight:700 }}>kakonbala.official@gmail.com</div>
                    </div>
                  </a>

                  {/* Facebook */}
                  <a href="https://facebook.com/kakonbala.official" target="_blank" rel="noreferrer"
                    style={{ display:"flex",alignItems:"center",gap:10,background:"#1877F2",borderRadius:14,padding:"12px 20px",textDecoration:"none",cursor:"pointer",boxShadow:"0 4px 15px rgba(0,0,0,0.2)",transition:"all 0.2s" }}
                    onMouseEnter={e=>e.currentTarget.style.transform="translateY(-3px)"}
                    onMouseLeave={e=>e.currentTarget.style.transform="translateY(0)"}>
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="white">
                    ...
                    <div style={{ textAlign:"left" }}>
                      <div style={{ fontSize:10,color:"rgba(255,255,255,0.8)",fontWeight:600 }}>Facebook</div>
                      <div style={{ fontSize:12,color:"#FFF",fontWeight:700 }}>@kakonbala.official</div>
                    </div>
                  </a>

                  {/* Instagram */}
                  <a href="https://instagram.com/kako_nbala" target="_blank" rel="noreferrer"
                    style={{ display:"flex",alignItems:"center",gap:10,background:"linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)",borderRadius:14,padding:"12px 20px",textDecoration:"none",cursor:"pointer",boxShadow:"0 4px 15px rgba(0,0,0,0.2)",transition:"all 0.2s" }}
                    onMouseEnter={e=>e.currentTarget.style.transform="translateY(-3px)"}
                    onMouseLeave={e=>e.currentTarget.style.transform="translateY(0)"}>
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="white">
                    ...
                    <div style={{ textAlign:"left" }}>
                      <div style={{ fontSize:10,color:"rgba(255,255,255,0.8)",fontWeight:600 }}>Instagram</div>
                      <div style={{ fontSize:12,color:"#FFF",fontWeight:700 }}>@kako_nbala</div>
                    </div>
                  </a>
                    ...
                    {catProducts.slice(0,4).map(p=>(
                      <div key={p.id} style={{ ...glass,overflow:"hidden",cursor:"pointer",transition:"transform 0.2s" }}
                        ...
                        <div style={{ height:130 }}>
                          <Carousel images={p.imageUrls&&p.imageUrls.length?p.imageUrls:[p.imageUrl]} emoji={p.emoji} height={200} />
                        </div>
                        <div style={{ padding:"8px 12px" }}>
              ...
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:showFilters?14:0 }}>
                <div style={{ display:"flex",gap:10,alignItems:"center",flexWrap:"wrap" }}>
                  <button onClick={()=>setShowFilters(f=>!f)}
                    style={{ display:"flex",alignItems:"center",gap:6,...btn,padding:"8px 18px",fontSize:13,background:showFilters?GRAD:"rgba(255,255,255,0.7)",color:showFilters?"#FFF":MED,border:`1.5px solid rgba(173,20,87,0.3)`,boxShadow:"none" }}>
                    🎛 Filters {(selectedSizes.length>0||(priceRange[0]>0||priceRange[1]<10000))&&<span style={{background:PRIMARY,color:"#FFF",borderRadius:"50%",width:18,height:18,fontSize:10,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800}}>{selectedSizes.length+(priceRange[0]>0||priceRange[1]<10000?1:0)}</span>}
                  </button>
                  {/* Sort */}
                  <select value={sortBy} onChange={e=>setSortBy(e.target.value)}
                    style={{ padding:"8px 14px",border:`1.5px solid rgba(173,20,87,0.25)`,borderRadius:20,fontSize:13,fontFamily:"inherit",background:"rgba(255,255,255,0.75)",color:DARK,cursor:"pointer",outline:"none" }}>
                  ...
                  {/* Active filter chips */}
                  {selectedSizes.map(s=>(
                    <span key={s} style={{ display:"flex",alignItems:"center",gap:4,background:"rgba(173,20,87,0.1)",border:`1px solid rgba(173,20,87,0.3)`,borderRadius:20,padding:"4px 12px",fontSize:12,color:PRIMARY,fontWeight:600 }}>
                      {s} <button onClick={()=>setSelectedSizes(prev=>prev.filter(x=>x!==s))} style={{background:"none",border:"none",cursor:"pointer",color:PRIMARY,fontSize:14,padding:0,lineHeight:1}}>×</button>
                    </span>
                  ))}
                  {(priceRange[0]>0||priceRange[1]<10000)&&(
                    <span style={{ display:"flex",alignItems:"center",gap:4,background:"rgba(173,20,87,0.1)",border:`1px solid rgba(173,20,87,0.3)`,borderRadius:20,padding:"4px 12px",fontSize:12,color:PRIMARY,fontWeight:600 }}>
                      ৳{priceRange[0]}–{priceRange[1]<10000?priceRange[1]:"∞"}
                      <button onClick={()=>setPriceRange([0,10000])} style={{background:"none",border:"none",cursor:"pointer",color:PRIMARY,fontSize:14,padding:0,lineHeight:1}}>×</button>
                    </span>
                  )}
                  {(selectedSizes.length>0||(priceRange[0]>0||priceRange[1]<10000))&&(
                    <button onClick={()=>{setSelectedSizes([]);setPriceRange([0,10000]);}} style={{background:"none",border:"none",cursor:"pointer",color:DANGER,fontSize:12,fontWeight:700,fontFamily:"inherit"}}>Clear all</button>
                  )}
                </div>
                <div style={{ fontSize:13,color:MED,fontWeight:600 }}>{visible.length} products</div>
              </div>

              {/* Expanded filter panel */}
              {showFilters&&(
                <div style={{ ...glass,padding:20,display:"grid",gridTemplateColumns:"1fr 1fr",gap:20 }}>
                  {/* Price Range */}
                  <div>
                    <div style={{ fontSize:13,fontWeight:800,color:DARK,marginBottom:12 }}>💰 Price Range</div>
                    <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
                      {[[0,500,"Under ৳500"],[500,1000,"৳500–1000"],[1000,2000,"৳1000–2000"],[2000,5000,"৳2000–5000"],[5000,10000,"৳5000+"]].map(([min,max,label])=>(
                        <button key={label} onClick={()=>setPriceRange([min,max])}
                          style={{ padding:"5px 14px",borderRadius:20,cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"inherit",
                            border:`1.5px solid ${priceRange[0]===min&&priceRange[1]===max?PRIMARY:"rgba(173,20,87,0.25)"}`,
                            background:priceRange[0]===min&&priceRange[1]===max?"rgba(173,20,87,0.1)":"rgba(255,255,255,0.7)",
                            color:priceRange[0]===min&&priceRange[1]===max?PRIMARY:MED }}>
                          {label}
                        </button>
                      ))}
                    </div>
                    <div style={{ marginTop:12 }}>
                      <div style={{ display:"flex",gap:8,alignItems:"center" }}>
                        <input type="number" placeholder="Min ৳" value={priceRange[0]||""} min={0}
                          onChange={e=>setPriceRange([Number(e.target.value)||0,priceRange[1]])}
                          style={{ width:"45%",padding:"6px 10px",border:`1.5px solid rgba(173,20,87,0.25)`,borderRadius:8,fontSize:12,fontFamily:"inherit",background:"rgba(255,255,255,0.85)" }}/>
                        <span style={{color:MED,fontSize:12}}>to</span>
                        <input type="number" placeholder="Max ৳" value={priceRange[1]===10000?"":priceRange[1]} min={0}
                  ...
                  {/* Size Filter */}
                  <div>
                    <div style={{ fontSize:13,fontWeight:800,color:DARK,marginBottom:12 }}>📏 Size</div>
                    {allSizes.length===0
                      ? <div style={{fontSize:12,color:LIGHT}}>No sizes available for current products</div>
                      : <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
                          {allSizes.map(s=>(
                            <button key={s} onClick={()=>setSelectedSizes(prev=>prev.includes(s)?prev.filter(x=>x!==s):[...prev,s])}
                              style={{ width:48,height:48,borderRadius:10,cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:"inherit",
                                border:`1.5px solid ${selectedSizes.includes(s)?PRIMARY:"rgba(173,20,87,0.25)"}`,
                                background:selectedSizes.includes(s)?"rgba(173,20,87,0.12)":"rgba(255,255,255,0.7)",
              ...
              {visible.map(p => (
                <div key={p.id} style={{ ...glass,overflow:"hidden",transition:"transform 0.2s",cursor:"pointer" }}
                  ...
                  <div onClick={() => setSelectedProduct(p)} style={{ position:"relative" }}>
                    <Carousel images={p.imageUrls&&p.imageUrls.length?p.imageUrls:[p.imageUrl]} emoji={p.emoji} height={260} />
