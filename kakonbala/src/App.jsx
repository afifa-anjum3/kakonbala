    ...
    <div style={{fontFamily:"'Hind Siliguri','Segoe UI',Arial,sans-serif",...pageBg,color:C.dark}}>
      ...
      {showAuth&&(
        <>
          ...
          <div style={{position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",
            width:380,background:"rgba(255,255,255,0.95)",backdropFilter:"blur(20px)",
            borderRadius:24,overflow:"hidden",zIndex:201,
            boxShadow:"0 24px 80px rgba(173,20,87,0.35)",border:"1px solid rgba(255,255,255,0.7)"}}>
            ...
            <div style={{display:"flex",borderBottom:"1px solid rgba(173,20,87,0.15)"}}>
              {[["login","Login"],["signup","Sign Up"]].map(([k,l])=>(
                <button key={k} onClick={()=>{setAuthTab(k);setAuthError("");}}
                  style={{flex:1,padding:"12px",border:"none",cursor:"pointer",
                    fontFamily:"inherit",fontSize:14,fontWeight:700,
                    background:authTab===k?"rgba(173,20,87,0.07)":"transparent",
                    color:authTab===k?C.primary:C.med,
                    borderBottom:authTab===k?`2px solid ${C.primary}`:"2px solid transparent"}}>
                  {l}
                </button>
              ))}
            </div>
            <div style={{padding:"24px 28px"}}>
              {authTab==="signup"&&(
                <div style={{marginBottom:14}}>
                  <label style={{fontSize:12,color:C.med,fontWeight:700,display:"block",marginBottom:4}}>Full Name</label>
                  <input style={inp} type="text" placeholder="Your name"
                    value={authName} onChange={e=>setAuthName(e.target.value)}/>
                </div>
              )}
              <div style={{marginBottom:14}}>
                <label style={{fontSize:12,color:C.med,fontWeight:700,display:"block",marginBottom:4}}>Email</label>
              ...
              <div style={{marginBottom:16}}>
                <label style={{fontSize:12,color:C.med,fontWeight:700,display:"block",marginBottom:4}}>Password</label>
                <input style={inp} type="password" placeholder="Min. 6 characters"
                  value={authPassword} onChange={e=>setAuthPassword(e.target.value)}
                  onKeyDown={e=>e.key==="Enter"&&(authTab==="login"?handleLogin():handleSignup())}/>
              </div>
              {authError&&(
                <div style={{background:C.dangerBg,color:C.danger,padding:"8px 12px",
              ...
              <button onClick={authTab==="login"?handleLogin:handleSignup}
                ...
                style={{...btn,width:"100%",padding:"13px",fontSize:15,opacity:authWorking?0.7:1}}>
                {authWorking?"Please wait...":(authTab==="login"?"Login →":"Create Account →")}
              </button>
              <div style={{fontSize:11,color:C.light,textAlign:"center",marginTop:12}}>
                {authTab==="login"?"Don't have an account? ":"Already have an account? "}
                <span onClick={()=>{setAuthTab(authTab==="login"?"signup":"login");setAuthError("");}}
                  style={{color:C.primary,fontWeight:700,cursor:"pointer",textDecoration:"underline"}}>
                  {authTab==="login"?"Sign Up":"Login"}
                </span>
              </div>
              {authTab==="login"&&(
                <div style={{fontSize:11,color:C.light,textAlign:"center",marginTop:8}}>
                  Or <span onClick={()=>{setUser({email:"guest"});setShowAuth(false);}} 
                    style={{color:C.med,cursor:"pointer",textDecoration:"underline"}}>continue as guest</span>
                </div>
              )}
            </div>
          </div>
