    ...
    >
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        @media (max-width: 640px) {
          ...
          /* Logo */
          header img[alt="logo"] { width: 36px !important; height: 36px !important; }
          /* Main padding */
          main { padding: 10px 12px !important; }
          /* Grids → 1 column */
          .kk-grid { grid-template-columns: 1fr !important; gap: 12px !important; }
          .kk-grid-2 { grid-template-columns: repeat(2,1fr) !important; gap: 8px !important; }
          /* Hero */
          .kk-hero-wrap { height: 220px !important; }
          .kk-hero-content { padding: 16px !important; max-width: 95% !important; }
          .kk-hero-h1 { font-size: 22px !important; }
          .kk-hero-h2 { font-size: 20px !important; }
          .kk-hero-tagline { font-size: 10px !important; }
          /* Search bar */
          .kk-search-cat { display: none !important; }
          .kk-search-tags { display: none !important; }
          /* Stats bar */
          .kk-stats { grid-template-columns: repeat(2,1fr) !important; }
          /* Modals - fullscreen */
          .kk-modal { width: 100vw !important; max-height: 100vh !important; border-radius: 0 !important; top: 0 !important; left: 0 !important; transform: none !important; }
          ...
          .kk-lang { display: none !important; }
          .kk-tagline-text { display: none !important; }
          /* Touch friendly */
        ...
        @media (min-width: 641px) and (max-width: 1023px) {
          ...
          .kk-grid { grid-template-columns: repeat(2,1fr) !important; }
          .kk-hero-wrap { height: 300px !important; }
          .kk-hero-h1 { font-size: 30px !important; }
          .kk-hero-h2 { font-size: 28px !important; }
          .kk-dash-grid { grid-template-columns: repeat(2,1fr) !important; }
          .kk-about { grid-template-columns: 1fr !important; }
          .kk-features { grid-template-columns: repeat(2,1fr) !important; }
          .kk-modal-grid { grid-template-columns: 1fr !important; }
                  ...
                  <div style={{ background:"rgba(255,248,255,0.95)",border:"1.5px solid rgba(173,20,87,0.2)",borderRadius:14,padding:16 }}>
                    ...
                    <div style={{ display:"flex",gap:8,marginBottom:14 }}>
                      {[{name:"bKash",color:"#E2136E"},{name:"Nagad",color:"#E8222E"},{name:"Rocket",color:"#8B1A8B"}].map(({name,color})=>(
                        <button key={name} onClick={()=>setSelectedGateway(name)}
                          style={{ flex:1,padding:"10px 6px",borderRadius:10,cursor:"pointer",fontFamily:"inherit",fontWeight:700,fontSize:13,
                            border:`2px solid ${selectedGateway===name?color:"rgba(0,0,0,0.08)"}`,
                            background:selectedGateway===name?color+"18":"#FFF",color:selectedGateway===name?color:"#666",
                            boxShadow:selectedGateway===name?`0 4px 14px ${color}44`:"none",transition:"all 0.2s" }}>
                          {name}
                        </button>
                      ))}
                    </div>
                    ...
                    <div style={{ background:"#FFF",borderRadius:10,padding:"12px 16px",marginBottom:14,border:"1px solid rgba(173,20,87,0.1)",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                      ...
                      <div style={{ width:40,height:40,borderRadius:10,background:selectedGateway==="bKash"?"#E2136E":selectedGateway==="Nagad"?"#E8222E":"#8B1A8B",display:"flex",alignItems:"center",justifyContent:"center" }}>
                        <span style={{ color:"#FFF",fontSize:18,fontWeight:900 }}>{selectedGateway[0]}</span>
                      </div>
                    </div>
      ...
      {showProfile&&(
        <>
          ...
          <div className="kk-modal" style={{ position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:"min(500px,95vw)",maxHeight:"90vh",overflowY:"auto",background:"rgba(255,255,255,0.97)",borderRadius:24,zIndex:201,boxShadow:"0 24px 80px rgba(173,20,87,0.3)" }}>
            ...
            <div style={{ padding:24 }}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16 }}>
                <div><label style={{ fontSize:11,color:MED,fontWeight:700,display:"block",marginBottom:3 }}>Full Name</label>
                  <input style={inp} type="text" placeholder="Your name" value={profileData.name||""} onChange={e=>setProfileData(p=>({...p,name:e.target.value}))}/></div>
                <div><label style={{ fontSize:11,color:MED,fontWeight:700,display:"block",marginBottom:3 }}>Phone</label>
                  ...
                  {profileData.phone&&validatePhone(profileData.phone)&&<div style={{ fontSize:10,color:SUCCESS,marginTop:2,fontWeight:600 }}>✓ Valid</div>}</div>
                <div style={{ gridColumn:"span 2" }}><label style={{ fontSize:11,color:MED,fontWeight:700,display:"block",marginBottom:3 }}>Email</label>
                  <input style={inp} type="email" placeholder="your@email.com" value={profileData.email||""} onChange={e=>setProfileData(p=>({...p,email:e.target.value}))}/></div>
              </div>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16 }}>
                <div><label style={{ fontSize:11,color:MED,fontWeight:700,display:"block",marginBottom:3 }}>District</label>
                  <select style={inp} value={profileData.district||""} onChange={e=>setProfileData(p=>({...p,district:e.target.value,area:"",thana:"",postOffice:""}))}>
                    <option value="">-- Select --</option>{BD_DISTRICTS.map(d=><option key={d} value={d}>{d}</option>)}</select></div>
                <div><label style={{ fontSize:11,color:MED,fontWeight:700,display:"block",marginBottom:3 }}>Area</label>
                  <select style={inp} value={profileData.area||""} onChange={e=>setProfileData(p=>({...p,area:e.target.value,thana:"",postOffice:""}))}>
                    <option value="">-- Select --</option>{getAreas(profileData.district).map(a=><option key={a} value={a}>{a}</option>)}</select></div>
                <div><label style={{ fontSize:11,color:MED,fontWeight:700,display:"block",marginBottom:3 }}>Thana</label>
                  <select style={inp} value={profileData.thana||""} onChange={e=>setProfileData(p=>({...p,thana:e.target.value,postOffice:""}))}>
                    <option value="">-- Select --</option>{getThanas(profileData.district,profileData.area).map(t2=><option key={t2} value={t2}>{t2}</option>)}</select></div>
                <div><label style={{ fontSize:11,color:MED,fontWeight:700,display:"block",marginBottom:3 }}>Post Office</label>
                  <select style={inp} value={profileData.postOffice||""} onChange={e=>setProfileData(p=>({...p,postOffice:e.target.value}))}>
                    <option value="">-- Select --</option>{getPostOffices(profileData.district,profileData.area,profileData.thana).map(po=><option key={po} value={po}>{po}</option>)}</select></div>
                ...
                {orders.filter(o=>o.customer?.phone===profileData.phone||o.customer?.email===profileData.email).length===0
                  ...
                  :orders.filter(o=>o.customer?.phone===profileData.phone||o.customer?.email===profileData.email).slice(0,5).map(o=>(
                    <div key={o.id} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:"rgba(255,255,255,0.7)",borderRadius:10,marginBottom:8,border:"1px solid rgba(173,20,87,0.08)" }}>
                      <div><div style={{ fontSize:12,fontWeight:700,color:DARK }}>{(o.items||[]).slice(0,2).map(i=>i.name).join(", ")}</div>
                      <div style={{ fontSize:10,color:MED }}>{o.createdAt?.seconds?new Date(o.createdAt.seconds*1000).toLocaleDateString():""}</div></div>
                      <div style={{ textAlign:"right" }}><div style={{ fontSize:13,fontWeight:800,color:PRIMARY }}>৳{(o.total||0).toLocaleString()}</div>
                      <span style={{ fontSize:10,padding:"2px 8px",borderRadius:8,fontWeight:700,background:"rgba(232,245,233,0.9)",color:SUCCESS }}>{(o.status||"").replace(/_/g," ")}</span></div>
                    </div>
                  ))}
              </div>
            </div>
